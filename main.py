...
#print(HOGMis.T)
#print(SURFMis.T)
#print(utils.countMis(HOGMis))
#print(utils.countMis(SURFMis))
    ...
    for x in np.linspace(0,10,25):
        ...
        for i in range(constants.numTestImages):
            ...
            if predFromProb[i] == SURFPredictions[i]:
                ...
                bestPrediction[i] = SURFPredictions[i]
        #    elif predFromProb[i] == HOGPredictions[i]:
        #        bestPrediction[i] = HOGPredictions[i]
...
count = 0
for i in range(74):
#    if HOGMis[i] and SURFMis[i] and predFromProbMis[i]:
    if bestMis[i]:
        count+=1
