...
class MergeComponents:
    ...
    def mergeNearbyComponents(self, connectedComponents):
        ...
        self.thresholdHorizontal = int(np.shape(self.cc.image)[0] * 0.1)
        self.thresholdVertical = int(np.shape(self.cc.image)[1] * 0.1)
        i = 0
    ...
    def removeSmallComponent(self):
        ...
        if self.numOfComponentBySize["medium"] > 1:
            self.removeSmallComponentFromList()
